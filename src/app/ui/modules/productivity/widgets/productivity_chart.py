"""Productivity Chart — Multi-line YoY% (quarterly, periods=4)."""
...
SERIES_ORDER: List[str] = ["Productivity", "Unit Labor Costs", "Real Compensation"]

SERIES_COLORS: Dict[str, str] = {
    "Unit Labor Costs": "#FF6B6B",
    "Real Compensation": "#42A5F5",
}

SERIES_VISIBILITY_KEYS: Dict[str, str] = {
    "Productivity": "show_productivity",
    "Unit Labor Costs": "show_ulc",
    "Real Compensation": "show_real_comp",
}


class ProductivityChart(BaseChart):
    """Multi-line productivity chart — always YoY% (quarterly)."""
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading productivity data...", self)
    ...
    def update_data(self, prod_df: "Optional[pd.DataFrame]",
        ...
        if prod_df is None or prod_df.empty:
            self.show_placeholder("No productivity data available.")
            return
        ...
        # Compute YoY% (quarterly: periods=4)
        yoy_data = {}
        for col in visible:
            if col in prod_df.columns:
                yoy = prod_df[col].pct_change(periods=4) * 100
                yoy_data[col] = yoy
        ...
        for name in SERIES_ORDER:
            ...
            color = accent if name == "Productivity" else SERIES_COLORS.get(name, "#888888")
