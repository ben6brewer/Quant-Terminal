"""Housing Permits Chart — Dual-mode: Raw stacked area or YoY% line."""
...
# Component colors
_SF_PERMITS_COLOR = "#AB47BC"       # Purple
_MULTI_PERMITS_COLOR = "#EF5350"    # Red


class HousingPermitsChart(BaseChart):
    """Housing permits chart — stacked area (Raw) or single YoY% line."""
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading housing permits data...", self)
    ...
    def update_data(
        self,
        permits_df: "Optional[pd.DataFrame]",
        ...
        if self._view_mode == "YoY %":
            self._render_yoy(permits_df, usrec_df, settings)
        else:
            self._render_stacked(permits_df, usrec_df, settings)

    # ── Raw: stacked area ─────────────────────────────────────────────────

    def _render_stacked(self, permits_df, usrec_df, settings):
        import pandas as pd

        if permits_df is None or permits_df.empty:
            self.show_placeholder("No housing permits data available.")
            return

        show_recession = settings.get("show_recession_bands", True)
        show_sf = settings.get("show_sf_permits", True)
        show_multi = settings.get("show_multi_permits", True)
        ...
        ref_col = None
        for col in ["SF Permits", "Multi-Family Permits"]:
        ...
        ref_series = permits_df[ref_col].dropna()
        if ref_series.empty:
            self.show_placeholder("No housing permits data available.")
            return
        ...
        self._sf_values = (
            permits_df["SF Permits"].reindex(ref_series.index).values.astype(float)
            if "SF Permits" in permits_df.columns else None
        )
        self._multi_values = (
        ...
        # Stacked area: SF Permits + Multi-Family Permits
        stacks = []
        if show_sf and self._sf_values is not None:
            stacks.append(("SF Permits", self._sf_values, _SF_PERMITS_COLOR))
        if show_multi and self._multi_values is not None:
            stacks.append(("Multi-Family Permits", self._multi_values, _MULTI_PERMITS_COLOR))
    ...
    # ── YoY %: single Total Permits line ──────────────────────────────────

    def _render_yoy(self, permits_df, usrec_df, settings):
        import pandas as pd

        if permits_df is None or permits_df.empty or "Total Permits" not in permits_df.columns:
            self.show_placeholder("No housing permits data available.")
            return

        show_recession = settings.get("show_recession_bands", True)

        yoy = permits_df["Total Permits"].pct_change(periods=12) * 100
        yoy = yoy.dropna()
        if yoy.empty:
            ...
            if self._yoy_values is not None and idx < len(self._yoy_values):
                ...
                if not np.isnan(val):
                    lines.append(
                        ...
                        f" Total Permits YoY: {val:+.1f}%"
                    )
        else:
            for label, vals, color in [
                ("SF Permits", self._sf_values, _SF_PERMITS_COLOR),
                ("Multi-Family", self._multi_values, _MULTI_PERMITS_COLOR),
            ]:
