"""Commercial Real Estate Chart — CRE price indices and loan volumes."""
...
_SERIES_SETTINGS = {
    "CRE Prices": "show_cre_prices",
}


class CommercialRealEstateChart(BaseChart):
    """CRE multi-line chart with Raw/YoY% modes."""
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading CRE data...", self)
    ...
    def update_data(self, cre_df: "Optional[pd.DataFrame]",
        ...
        if self._view_mode == "YoY %":
            if cre_df is not None and not cre_df.empty:
                yoy = cre_df.pct_change(periods=4) * 100
                yoy = yoy.dropna(how="all")
                self._render_lines(yoy, usrec_df, settings, suffix=" YoY")
            else:
                self.show_placeholder("No CRE data available.")
        else:
            self._render_lines(cre_df, usrec_df, settings)
