"""Household Debt Toolbar — Home, lookback, view toggle, stats."""
...
class HouseholdDebtToolbar(FredToolbar):
    """Household Debt toolbar — view toggle + stats."""
    ...
    def update_info(self, household_debt=None, debt_to_gdp=None, **kwargs):
        if household_debt is not None:
            self.debt_label.setText(f"Debt: ${household_debt:.1f}T")
        if debt_to_gdp is not None:
            self.gdp_label.setText(f"Debt/GDP: {debt_to_gdp:.1f}%")
