"""Household Debt Chart — Dual-mode: Raw debt, % GDP, Debt Service, YoY."""
...
class HouseholdDebtChart(BaseChart):
    """Household debt chart — single line per view mode."""
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading household debt data...", self)
    ...
    def update_data(self, debt_raw_df: "Optional[pd.DataFrame]",
                    debt_ratios_df: "Optional[pd.DataFrame]",
        ...
        if self._view_mode == "% GDP":
            ...
            self._render_single(debt_ratios_df, usrec_df, settings, "Debt-to-GDP", "%")
        elif self._view_mode == "Debt Service":
            self.plot_item.setAxisItems({"right": self._pct_axis})
            self._render_single(debt_ratios_df, usrec_df, settings, "Debt Service", "%")
        elif self._view_mode == "YoY":
            self._render_yoy(debt_raw_df, usrec_df, settings, "Household Debt", "Debt YoY")
        else:
            self.plot_item.setAxisItems({"right": self._right_axis})
            self._render_single(debt_raw_df, usrec_df, settings, "Household Debt", "T")

    def _render_single(self, df, usrec_df, settings, col_name, unit):
        import pandas as pd

        if df is None or df.empty or col_name not in df.columns:
            self.show_placeholder("No household debt data available.")
            return
    ...
    def _update_tooltip(self, idx: int, mouse_pos):
        ...
        for name, vals in self._series_values.items():
            ...
            elif self._view_mode == "% GDP":
                lines.append(f'<span style="color:{color_str};">■</span> Debt/GDP: {v:.1f}%')
            elif self._view_mode == "Debt Service":
                lines.append(f'<span style="color:{color_str};">■</span> Debt Service: {v:.1f}%')
