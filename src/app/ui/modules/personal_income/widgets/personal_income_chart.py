"""Personal Income Chart — Multi-line income levels with Nominal/Real toggle."""
...
INCOME_COLORS: Dict[str, str] = {
    "Disposable Income": "#66BB6A",
    "PCE Level": "#FF7043",
    "Real Disposable Income": "#66BB6A",
    "Real PCE Level": "#FF7043",
}

INCOME_ORDER: List[str] = ["Personal Income", "Disposable Income", "PCE Level"]
REAL_INCOME_ORDER: List[str] = ["Real Personal Income", "Real Disposable Income", "Real PCE Level"]


class PersonalIncomeChart(BaseChart):
    """Personal income chart — multi-line with Nominal/Real toggle."""
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading personal income data...", self)
    ...
    def update_data(self, income_df: "Optional[pd.DataFrame]",
                    real_income_df: "Optional[pd.DataFrame]",
        ...
        active_df = real_income_df if self._data_mode == "Real" else income_df
        order = REAL_INCOME_ORDER if self._data_mode == "Real" else INCOME_ORDER

        if self._view_mode == "YoY %":
            self._render_yoy(active_df, usrec_df, settings, order)
        else:
            self._render_income(active_df, usrec_df, settings, order)

    def _render_income(self, income_df, usrec_df, settings, order=None):
        import pandas as pd
        ...
        if income_df is None or income_df.empty:
            self.show_placeholder("No income data available.")
            return
        ...
        for name in order:
            if name not in income_df.columns:
                continue
            vals = income_df[name].reindex(ref_series.index).values.astype(float)
            self._series_values[name] = vals
            color = accent if name == primary else INCOME_COLORS.get(name, "#888888")
    ...
    def _render_yoy(self, income_df, usrec_df, settings, order=None):
        import pandas as pd
        ...
        # Find first valid series to use as date reference
        ref_series = None
        for name in order:
            if name in income_df.columns:
                s = income_df[name].dropna()
        ...
        for name in order:
            if name not in income_df.columns:
                continue
            yoy = income_df[name].pct_change(periods=12) * 100
            yoy = yoy.dropna()
            if not yoy.empty:
                yoy_data[name] = yoy
        ...
        # Use the first series' index as reference
        first_key = next(iter(yoy_data))
        ref_yoy = yoy_data[first_key]
        ...
        primary = order[0] if order else None
        for name in order:
            if name not in yoy_data:
                continue
            vals = yoy_data[name].reindex(ref_yoy.index).values.astype(float)
            self._series_values[name] = vals
            color = accent if name == primary else INCOME_COLORS.get(name, "#888888")
            line = self.plot_item.plot(x, vals, pen=pg.mkPen(color=color, width=2), name=name)
            line.setClipToView(True)
    ...
    def _update_tooltip(self, idx: int, mouse_pos):
        ...
        for name, vals in self._series_values.items():
            ...
            if np.isnan(v):
                continue
            if name in ("Personal Income", "Real Personal Income"):
                color_str = f"rgb({accent[0]},{accent[1]},{accent[2]})"
            else:
                color_str = INCOME_COLORS.get(name, "#888888")
            if self._view_mode == "YoY %":
    ...
    def set_theme(self, theme: str):
        ...
        for name, line in self._line_items.items():
            if name in ("Personal Income", "Real Personal Income"):
                ...
                line.setPen(pg.mkPen(color=INCOME_COLORS.get(name, "#888888"), width=2))
        self._placeholder.setStyleSheet("font-size: 16px; color: #888888; background: transparent;")
