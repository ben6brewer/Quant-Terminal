"""Personal Income Chart — Dual-mode: Income multi-line or Savings Rate line."""
...
class PersonalIncomeChart(BaseChart):
    """Personal income chart — multi-line (Income) or single (Savings)."""
    ...
    def update_data(self, income_df: "Optional[pd.DataFrame]",
                    savings_df: "Optional[pd.DataFrame]",
        ...
        self._view_mode = settings.get("view_mode", "Income")

        if self._view_mode == "Savings":
            self._render_savings(savings_df, usrec_df, settings)
        else:
            self._render_income(income_df, usrec_df, settings)

    def _render_income(self, income_df, usrec_df, settings):
        import pandas as pd
        ...
        for name in INCOME_ORDER:
            ...
            color = accent if name == "Personal Income" else INCOME_COLORS.get(name, "#888888")
    ...
    def _render_savings(self, savings_df, usrec_df, settings):
        import pandas as pd

        if savings_df is None or savings_df.empty or "Savings Rate" not in savings_df.columns:
            self.show_placeholder("No savings rate data available.")
            return
        ...
        self._series_values = {"Savings Rate": vals}
        ...
        line = self.plot_item.plot(x, vals, pen=pg.mkPen(color=accent, width=2.5))
        line.setClipToView(True)
        self._line_items["Savings Rate"] = line
    ...
    def _update_tooltip(self, idx: int, mouse_pos):
        ...
        for name, vals in self._series_values.items():
            ...
            if np.isnan(v):
                continue
            if name in ("Personal Income", "Savings Rate"):
                color_str = f"rgb({accent[0]},{accent[1]},{accent[2]})"
            else:
                color_str = INCOME_COLORS.get(name, "#888888")
            if name == "Savings Rate":
                lines.append(f'<span style="color:{color_str};">\u25a0</span> Savings Rate: {v:.1f}%')
            else:
    ...
    def set_theme(self, theme: str):
        ...
        for name, line in self._line_items.items():
            if name in ("Personal Income", "Savings Rate"):
                line.setPen(pg.mkPen(color=accent, width=2.5 if name == "Savings Rate" else 2))
            else:
