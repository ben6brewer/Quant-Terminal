"""Currency Chart — Dual-mode: Raw multi-line or YoY% lines."""
...
CURRENCY_COLORS: Dict[str, str] = {
    "Adv. Economies": "#66BB6A",
}

CURRENCY_SERIES_ORDER: List[str] = ["Dollar Index", "Adv. Economies"]

SERIES_VISIBILITY_KEYS: Dict[str, str] = {
    "Dollar Index": "show_dollar_index",
    "Adv. Economies": "show_adv_economies",
}


class CurrencyChart(BaseChart):
    """Multi-line currency chart — Raw or YoY%."""
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading currency data...", self)
    ...
    def update_data(self, dollar_df: "Optional[pd.DataFrame]",
        ...
        if self._view_mode == "YoY %":
            self._render_yoy(dollar_df, usrec_df, settings)
        else:
            self._render_raw(dollar_df, usrec_df, settings)

    # -- Raw: multi-line -------------------------------------------------

    def _render_raw(self, dollar_df, usrec_df, settings):
        import pandas as pd

        if dollar_df is None or dollar_df.empty:
            self.show_placeholder("No currency data available.")
            return

        show_recession = settings.get("show_recession_bands", True)

        visible = [n for n in CURRENCY_SERIES_ORDER
        ...
        ref_series = None
        for name in visible:
            if name in dollar_df.columns:
                s = dollar_df[name].dropna()
                if not s.empty:
                    ref_series = s
                    break
        ...
        all_vals = []
        for name in CURRENCY_SERIES_ORDER:
            if name not in visible or name not in dollar_df.columns:
                continue
            vals = dollar_df[name].reindex(ref_series.index).values.astype(float)
            self._series_values[name] = vals
            color = accent if name == "Dollar Index" else CURRENCY_COLORS.get(name, "#888888")
    ...
    # -- YoY %: pct_change on daily data ---------------------------------

    def _render_yoy(self, dollar_df, usrec_df, settings):
        import pandas as pd
        ...
        for col in visible:
            if col in dollar_df.columns:
                yoy = dollar_df[col].pct_change(periods=252) * 100
                yoy_data[col] = yoy
    ...
    def _update_tooltip(self, idx: int, mouse_pos):
        ...
        for name in CURRENCY_SERIES_ORDER:
            ...
            if name == "Dollar Index":
                ...
                color_str = CURRENCY_COLORS.get(name, "#888888")
            if self._view_mode == "YoY %":
    ...
    def set_theme(self, theme: str):
        ...
        for name, line in self._line_items.items():
            if name == "Dollar Index":
                ...
                line.setPen(pg.mkPen(color=CURRENCY_COLORS.get(name, "#888888"), width=2))
        self._placeholder.setStyleSheet("font-size: 16px; color: #888888; background: transparent;")
