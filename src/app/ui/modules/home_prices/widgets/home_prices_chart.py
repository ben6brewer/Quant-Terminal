"""Home Prices Chart — Median and average sale prices."""
...
_SERIES_COLORS = {
    "Median Sale Price": "#FFB74D",
    "Average Sale Price": "#CE93D8",
}

_SERIES_SETTINGS = {
    "Median Sale Price": "show_median_price",
    "Average Sale Price": "show_avg_price",
}


class HomePricesChart(BaseChart):
    """Home prices multi-line chart with Raw/YoY% modes."""
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading home price data...", self)
    ...
    def update_data(self, sale_prices_df: "Optional[pd.DataFrame]",
        ...
        if sale_prices_df is None or sale_prices_df.empty:
            self.show_placeholder("No home price data available.")
            return
        combined = sale_prices_df

        if self._view_mode == "YoY %":
            yoy = combined.pct_change(periods=12) * 100
            yoy = yoy.dropna(how="all")
            self._render_lines(yoy, usrec_df, settings, suffix=" YoY")
        else:
            self._render_lines(combined, usrec_df, settings)
