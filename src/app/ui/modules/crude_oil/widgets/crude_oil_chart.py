"""Crude Oil Chart — Dual-mode: Raw multi-line or YoY% lines."""
...
CRUDE_COLORS: Dict[str, str] = {
    "Brent Crude": "#66BB6A",
}

CRUDE_SERIES_ORDER: List[str] = ["WTI Crude", "Brent Crude"]
...
class CrudeOilChart(BaseChart):
    """Multi-line crude oil chart — Raw or YoY%."""
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading crude oil data...", self)
    ...
    def _render_raw(self, energy_df, usrec_df, settings):
        ...
        if energy_df is None or energy_df.empty:
            self.show_placeholder("No crude oil data available.")
            return

        show_recession = settings.get("show_recession_bands", True)

        visible = [n for n in CRUDE_SERIES_ORDER
        ...
        all_vals = []
        for name in CRUDE_SERIES_ORDER:
            ...
            color = accent if name == "WTI Crude" else CRUDE_COLORS.get(name, "#888888")
    ...
    def _render_yoy(self, energy_df, usrec_df, settings):
        ...
        for col in visible:
            if col in energy_df.columns:
                yoy = energy_df[col].pct_change(periods=252) * 100
                yoy_data[col] = yoy
    ...
    def _update_tooltip(self, idx: int, mouse_pos):
        ...
        for name in CRUDE_SERIES_ORDER:
            ...
            if name == "WTI Crude":
                ...
                color_str = CRUDE_COLORS.get(name, "#888888")
            if self._view_mode == "YoY %":
                lines.append(f'<span style="color:{color_str};">\u25a0</span> {name}: {v:+.1f}%')
            else:
                lines.append(f'<span style="color:{color_str};">\u25a0</span> {name}: ${v:.2f}/bbl')
    ...
    def set_theme(self, theme: str):
        ...
        for name, line in self._line_items.items():
            if name == "WTI Crude":
                ...
                line.setPen(pg.mkPen(color=CRUDE_COLORS.get(name, "#888888"), width=2))
        self._placeholder.setStyleSheet("font-size: 16px; color: #888888; background: transparent;")
