"""Student Loans Chart — Total and federal student loan balances."""
...
_SERIES_COLORS = {
    "Total Student Loans": "#4FC3F7",
    "Federal Student Loans": "#66BB6A",
}

_SERIES_SETTINGS = {
    "Total Student Loans": "show_total_loans",
    "Federal Student Loans": "show_federal_loans",
}


class StudentLoansChart(BaseChart):
    """Student loans chart — Raw ($T) or YoY % mode."""
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading student loan data...", self)
    ...
    def update_data(self, loans_df: "Optional[pd.DataFrame]",
        ...
        if self._view_mode == "YoY %" and loans_df is not None and not loans_df.empty:
            growth = loans_df.pct_change(periods=4) * 100
            growth = growth.dropna(how="all")
            self._render_lines(growth, usrec_df, settings, suffix=" YoY")
        else:
            self._render_lines(loans_df, usrec_df, settings)

    def _render_lines(self, df, usrec_df, settings, suffix=""):
        import pandas as pd

        if df is None or df.empty:
            self.show_placeholder("No student loan data available.")
            return
