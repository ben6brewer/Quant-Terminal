"""Wealth Distribution Chart — Stacked area of wealth shares by percentile."""
...
_STACK_ORDER = ["Bottom 50%", "50th-90th", "90th-99th", "Top 1%"]

_SERIES_SETTINGS = {
    "Top 1%": "show_top1",
    "90th-99th": "show_90_99",
    "50th-90th": "show_50_90",
    "Bottom 50%": "show_bottom50",
}


class WealthDistributionChart(BaseChart):
    """Wealth distribution chart — stacked area or line mode."""
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading wealth distribution data...", self)
    ...
    def update_data(self, wealth_df: "Optional[pd.DataFrame]",
        ...
        self._view_mode = settings.get("view_mode", "Stacked Area")

        if self._view_mode == "Line":
            self._render_line(wealth_df, usrec_df, settings)
        else:
            self._render_stacked(wealth_df, usrec_df, settings)

    def _render_stacked(self, df, usrec_df, settings):
        import pandas as pd

        if df is None or df.empty:
            self.show_placeholder("No wealth distribution data available.")
            return

        show_recession = settings.get("show_recession_bands", True)
        cols = [c for c in _STACK_ORDER if c in df.columns
                and settings.get(_SERIES_SETTINGS.get(c, ""), True)]
        if not cols:
            self.show_placeholder("No wealth distribution data available.")
            return

        ref = df[cols[0]].dropna()
        if ref.empty:
            self.show_placeholder("No wealth distribution data available.")
            return
        ...
        for col in cols:
            vals = df[col].reindex(ref.index).values.astype(float)
            safe_vals = np.nan_to_num(vals, nan=0.0)
            self._series_values[col] = vals
            color = QColor(_SERIES_COLORS[col])
            ...
            lower = self.plot_item.plot(x, cumsum, pen=pg.mkPen(None))
            upper = self.plot_item.plot(x, top, pen=fill_pen, name=col)
            fill = pg.FillBetweenItem(lower, upper, brush=fill_brush)
            self.plot_item.addItem(fill)
            self._area_items.extend([lower, upper, fill])
            cumsum = top

        y_max = float(np.nanmax(cumsum)) if len(cumsum) > 0 else 100
        self.plot_item.setYRange(0, y_max * 1.05, padding=0)
    ...
    def _render_line(self, df, usrec_df, settings):
        self._render_lines(df, usrec_df, settings)
    ...
    def _update_tooltip(self, idx: int, mouse_pos):
        ...
        for name, vals in self._series_values.items():
            ...
            if np.isnan(v):
                continue
            base_name = name.replace(" YoY", "")
            color_str = _SERIES_COLORS.get(base_name, "#888888")
            if "YoY" in name or "Growth" in name:
