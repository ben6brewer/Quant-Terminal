...
from ..widgets.demographics_chart import DEMO_LABELS, DEMO_COLORS


class DemographicsSettingsDialog(ThemedDialog):
    """Settings dialog for the Demographics module."""

    def __init__(self, theme_manager, current_settings: dict, parent=None):
        self._current_settings = current_settings
        self.result = None
        super().__init__(theme_manager, "Demographics Settings", parent, min_width=420)
    ...
    def _setup_content(self, layout):
        ...
        active_series = set(
            self._current_settings.get("demo_series", ["White", "Black", "Hispanic"])
        )
        self._demo_checkboxes = {}
        demo_grid = QGridLayout()
        demo_grid.setSpacing(6)
        for i, name in enumerate(DEMO_LABELS):
            cb = QCheckBox(name)
            cb.setChecked(name in active_series)
            row, col = divmod(i, 2)
            demo_grid.addWidget(cb, row, col)
            self._demo_checkboxes[name] = cb
    ...
    def _save_settings(self):
        demo_series = [name for name, cb in self._demo_checkboxes.items() if cb.isChecked()]
        self.result = {
