"""Demographics Chart - Unemployment rates by race and gender."""
...
DEMO_LABELS = list(DEMO_COLORS.keys())


class DemographicsChart(BaseChart):
    """Multi-line demographics chart with recession shading."""
    ...
    def __init__(self, parent=None):
        ...
        self._visible_series: List[str] = DEMO_LABELS
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading demographics data...", self)
    ...
    def update_data(
        ...
        if rates_df is None or rates_df.empty:
            self.show_placeholder("No demographics data available.")
            return

        available = [c for c in DEMO_LABELS if c in rates_df.columns]
        if not available:
            self.show_placeholder("No demographic series available.")
            return
        ...
        self._visible_series = settings.get("demo_series", DEMO_LABELS)

        subset = rates_df[available].dropna(how="all")
        if subset.empty:
            self.show_placeholder("Insufficient demographics data.")
            return
        ...
        for name in available:
            ...
            color = DEMO_COLORS.get(name, "#888888")
