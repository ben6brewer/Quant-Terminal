"""Wage Growth Chart — Dual-mode: Raw levels or YoY% growth."""
...
WAGE_COLORS: Dict[str, str] = {
    "ECI YoY%": "#66BB6A",
    "ECI Wages": "#66BB6A",
}

RAW_SERIES_ORDER: List[str] = ["Avg Hourly Earnings", "ECI Wages"]
YOY_SERIES_ORDER: List[str] = ["AHE YoY%", "ECI YoY%"]

# Map settings keys to series names per view mode
_SERIES_VISIBILITY = {
    "show_ahe": {"Raw": "Avg Hourly Earnings", "YoY %": "AHE YoY%"},
    "show_eci": {"Raw": "ECI Wages", "YoY %": "ECI YoY%"},
}


class WageGrowthChart(BaseChart):
    """Multi-line wage growth chart with Raw / YoY% toggle."""
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading wage growth data...", self)
    ...
    def _visible_series(self, settings: dict, view_mode: str) -> set:
        """Return set of series names that should be shown."""
        visible = set()
        for key, mapping in _SERIES_VISIBILITY.items():
            if settings.get(key, True):
                name = mapping.get(view_mode)
                if name:
                    visible.add(name)
        return visible

    def update_data(self, wages_raw_df: "Optional[pd.DataFrame]",
                    wages_yoy_df: "Optional[pd.DataFrame]",
        ...
        self._view_mode = settings.get("view_mode", "YoY %")

        if self._view_mode == "Raw":
            self._render_raw(wages_raw_df, usrec_df, settings)
        else:
            self._render_yoy(wages_yoy_df, usrec_df, settings)

    def _render_raw(self, wages_raw_df, usrec_df, settings):
        import pandas as pd

        if wages_raw_df is None or wages_raw_df.empty:
            self.show_placeholder("No wage data available.")
            return

        show_recession = settings.get("show_recession_bands", True)
        visible = self._visible_series(settings, "Raw")

        ref_series = None
        for name in RAW_SERIES_ORDER:
            if name in wages_raw_df.columns and name in visible:
                s = wages_raw_df[name].dropna()
                if not s.empty:
                    ref_series = s
                    break
        if ref_series is None:
            self.show_placeholder("No wage data available (all series hidden).")
            return
        ...
        all_vals = []
        for name in RAW_SERIES_ORDER:
            if name not in wages_raw_df.columns or name not in visible:
                continue
            vals = wages_raw_df[name].reindex(ref_series.index).values.astype(float)
            self._series_values[name] = vals
            color = accent if name == "Avg Hourly Earnings" else WAGE_COLORS.get(name, "#888888")
    ...
    def _render_yoy(self, wages_yoy_df, usrec_df, settings):
        import pandas as pd

        if wages_yoy_df is None or wages_yoy_df.empty:
            self.show_placeholder("No wage growth data available.")
            return

        show_recession = settings.get("show_recession_bands", True)
        visible = self._visible_series(settings, "YoY %")

        ref_series = None
        for name in YOY_SERIES_ORDER:
            if name in wages_yoy_df.columns and name in visible:
                s = wages_yoy_df[name].dropna()
                if not s.empty:
                    ref_series = s
                    break
        if ref_series is None:
            self.show_placeholder("No wage growth data available (all series hidden).")
            return
        ...
        all_vals = []
        for name in YOY_SERIES_ORDER:
            if name not in wages_yoy_df.columns or name not in visible:
                continue
            vals = wages_yoy_df[name].reindex(ref_series.index).values.astype(float)
            self._series_values[name] = vals
            color = accent if name == "AHE YoY%" else WAGE_COLORS.get(name, "#888888")
            line = self.plot_item.plot(x, vals, pen=pg.mkPen(color=color, width=2), name=name)
            line.setClipToView(True)
            self._line_items[name] = line
            valid = vals[~np.isnan(vals)]
    ...
    def _update_tooltip(self, idx: int, mouse_pos):
        ...
        series_order = RAW_SERIES_ORDER if self._view_mode == "Raw" else YOY_SERIES_ORDER
        for name in series_order:
            ...
            if np.isnan(v):
                continue
            is_primary = name in ("AHE YoY%", "Avg Hourly Earnings")
            if is_primary:
                color_str = f"rgb({accent[0]},{accent[1]},{accent[2]})"
            else:
                color_str = WAGE_COLORS.get(name, "#888888")
            if self._view_mode == "Raw":
                if name == "Avg Hourly Earnings":
                    lines.append(f'<span style="color:{color_str};">\u25a0</span> {name}: ${v:.2f}/hr')
                else:
    ...
    def set_theme(self, theme: str):
        ...
        for name, line in self._line_items.items():
            ...
            if is_primary:
                ...
                line.setPen(pg.mkPen(color=WAGE_COLORS.get(name, "#888888"), width=2))
        self._placeholder.setStyleSheet("font-size: 16px; color: #888888; background: transparent;")
