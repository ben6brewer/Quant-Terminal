"""Income Inequality Chart — GINI index and median incomes (dual-axis)."""
...
_SERIES_COLORS = {
    "Median HH Income": "#4FC3F7",
    "Median Personal Income": "#81C784",
}

_SERIES_SETTINGS = {
    "Median HH Income": "show_hh_income",
    "Median Personal Income": "show_personal_income",
}


class IncomeInequalityChart(BaseChart):
    """Income inequality dual-axis chart — GINI (left) and Income (right)."""
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading income inequality data...", self)
    ...
    def update_data(self, incomes_df: "Optional[pd.DataFrame]",
        ...
        if self._view_mode == "YoY %":
            self._render_yoy(incomes_df, usrec_df, settings)
        else:
            self._render_raw(incomes_df, usrec_df, settings)

    def _render_raw(self, df, usrec_df, settings, suffix=""):
        import pandas as pd

        if df is None or df.empty:
            self.show_placeholder("No income inequality data available.")
            return
    ...
    def _render_yoy(self, df, usrec_df, settings):
        ...
        # Compute YoY % (annual data, so periods=1)
        yoy_df = df[visible_cols].pct_change(periods=1) * 100
        ...
        for col in visible_cols:
            vals = yoy_df[col].values.astype(float)
            label = f"{col} YoY"
            self._series_values[label] = vals
            color = _SERIES_COLORS[col]
            line = self.plot_item.plot(x, vals, pen=pg.mkPen(color=color, width=2), name=label)
