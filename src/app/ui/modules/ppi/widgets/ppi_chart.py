"""PPI Chart - Multi-line: PPI Final Demand, Core, Energy, Services."""
...
# Fixed colors per series (accent = PPI Final Demand, set at render time)
PPI_COLORS: Dict[str, str] = {
    "PPI Core":     "#EF5350",   # Red
    "PPI Energy":   "#FF7043",   # Orange
    "PPI Services": "#4FC3F7",   # Light blue
}

PPI_SERIES_ORDER: List[str] = [
    "PPI Final Demand", "PPI Core", "PPI Energy", "PPI Services"
]


class PpiChart(BaseChart):
    """Multi-line PPI chart with 4 series."""
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading PPI data...", self)
    ...
    # Map series name → settings visibility key
    SERIES_VISIBILITY_KEYS = {
        "PPI Final Demand": "show_ppi_final_demand",
        "PPI Core":         "show_ppi_core",
        "PPI Energy":       "show_ppi_energy",
        "PPI Services":     "show_ppi_services",
    }

    def update_data(self, ppi_df: "Optional[pd.DataFrame]", settings: dict):
        import pandas as pd

        if ppi_df is None or ppi_df.empty:
            self.show_placeholder("No PPI data available.")
            return
        ...
        visible_series = [
            name for name in PPI_SERIES_ORDER
            if settings.get(self.SERIES_VISIBILITY_KEYS.get(name, ""), True)
        ]
        ...
        # Find reference index from the first visible series that has data
        ref_series = None
        for name in visible_series:
            if name in ppi_df.columns:
                s = ppi_df[name].dropna()
                if not s.empty:
                    ref_series = s
                    break
        ...
        for name in PPI_SERIES_ORDER:
            if name not in visible_series or name not in ppi_df.columns:
                continue
            aligned = ppi_df[name].reindex(ref_series.index)
            vals = aligned.values.astype(float)
            self._series_values[name] = vals

            color = accent if name == "PPI Final Demand" else PPI_COLORS.get(name, "#888888")

            line = self.plot_item.plot(
                x, vals, pen=pg.mkPen(color=color, width=2), name=name
            )
    ...
    def _on_mouse_move(self, ev):
        if not self._dates is not None and len(self._dates) > 0:
            return
    ...
    def _update_tooltip(self, idx: int, mouse_pos):
        ...
        for name in PPI_SERIES_ORDER:
            ...
            if np.isnan(v):
                continue
            if name == "PPI Final Demand":
                color_str = f"rgb({accent[0]},{accent[1]},{accent[2]})"
            else:
                color_str = PPI_COLORS.get(name, "#888888")
            lines.append(
    ...
    def set_theme(self, theme: str):
        ...
        for name, line in self._line_items.items():
            if name == "PPI Final Demand":
                ...
                color = PPI_COLORS.get(name, "#888888")
                line.setPen(pg.mkPen(color=color, width=2))
        self._placeholder.setStyleSheet("font-size: 16px; color: #888888; background: transparent;")
