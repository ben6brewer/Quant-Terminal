...
# Fixed colors per series (accent = Industrial Production, set at render time)
IP_COLORS: Dict[str, str] = {
    "Manufacturing": "#EF5350",        # Red
    "Capacity Utilization": "#FFA726", # Orange
}

IP_SERIES_ORDER: List[str] = [
...
SERIES_VISIBILITY_KEYS = {
    ...
    "Capacity Utilization": "show_capacity_utilization",
}
...
class IndustrialProductionChart(BaseChart):
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading industrial production data...", self)
    ...
    def update_data(
        ...
        for name in IP_SERIES_ORDER:
            if name not in visible_series or name not in merged.columns:
                continue
            aligned = merged[name].reindex(ref_series.index)
            ...
            pen_style = Qt.DashLine if name == "Capacity Utilization" else Qt.SolidLine
    ...
    def set_theme(self, theme: str):
        ...
        for name, line in self._line_items.items():
            if name == "Industrial Production":
                ...
                color = IP_COLORS.get(name, "#888888")
                pen_style = Qt.DashLine if name == "Capacity Utilization" else Qt.SolidLine
                line.setPen(pg.mkPen(color=color, width=2, style=pen_style))
        self._placeholder.setStyleSheet("font-size: 16px; color: #888888; background: transparent;")
