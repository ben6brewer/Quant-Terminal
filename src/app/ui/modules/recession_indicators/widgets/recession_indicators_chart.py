"""Recession Indicators Chart — Multi-line: Recession Probability + Sahm Rule."""
...
SERIES_ORDER: List[str] = ["Recession Prob", "Sahm Rule"]
...
SERIES_VISIBILITY_KEYS: Dict[str, str] = {
    "Recession Prob": "show_recession_prob",
    "Sahm Rule": "show_sahm",
}


class RecessionIndicatorsChart(BaseChart):
    """Multi-line recession indicators chart."""
    ...
    def __init__(self, parent=None):
        ...
        self._sahm_threshold_line = None
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading recession indicator data...", self)
    ...
    def update_data(self, recession_df: "Optional[pd.DataFrame]",
        ...
        if recession_df is None or recession_df.empty:
            self.show_placeholder("No recession indicator data available.")
            return

        show_recession = settings.get("show_recession_bands", True)

        # Determine visible series based on settings
        visible_series = []
        for name in SERIES_ORDER:
            vis_key = SERIES_VISIBILITY_KEYS.get(name)
            if vis_key and settings.get(vis_key, True) and name in recession_df.columns:
                visible_series.append(name)

        # Find reference series for x-axis
        ref_series = None
        for name in SERIES_ORDER:
            if name in recession_df.columns:
                s = recession_df[name].dropna()
                if not s.empty:
                    ref_series = s
                    break
        ...
        # Add Sahm threshold line at 0.50
        sahm_pen = pg.mkPen(color="#FF0000", width=1, style=Qt.DashLine)
        self._sahm_threshold_line = pg.InfiniteLine(
            pos=0.50, angle=0, pen=sahm_pen, label="Sahm 0.50",
            labelOpts={"color": "#FF0000", "position": 0.95}
        )
        self.plot_item.addItem(self._sahm_threshold_line)

        all_vals = []
        for name in SERIES_ORDER:
            if name not in recession_df.columns:
                continue
            vals = recession_df[name].reindex(ref_series.index).values.astype(float)
            self._series_values[name] = vals

            vis_key = SERIES_VISIBILITY_KEYS.get(name)
            if vis_key and not settings.get(vis_key, True):
                continue

            color = accent if name == "Recession Prob" else SERIES_COLORS.get(name, "#888888")
    ...
    def _update_tooltip(self, idx: int, mouse_pos):
        ...
        for name in SERIES_ORDER:
            ...
            if np.isnan(v):
                continue
            if name == "Recession Prob":
                color_str = f"rgb({accent[0]},{accent[1]},{accent[2]})"
                lines.append(f'<span style="color:{color_str};">\u25a0</span> {name}: {v:.2f}%')
            else:
