"""Energy Production Chart — Oil/gas extraction, crude stocks, propane."""
...
_SERIES_COLORS = {
    "Mining Production": "#4FC3F7",
    "WTI Crude": "#FFA726",
}

_SERIES_SETTINGS = {
    "Mining Production": "show_mining",
    "WTI Crude": "show_wti",
}


class EnergyProductionChart(BaseChart):
    """Energy production multi-line chart — Raw or YoY% mode."""
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading energy production data...", self)
    ...
    def update_data(self, prod_df: "Optional[pd.DataFrame]",
        ...
        if self._view_mode == "YoY %" and prod_df is not None and not prod_df.empty:
            yoy = prod_df.pct_change(periods=12) * 100
            yoy = yoy.dropna(how="all")
            self._render_lines(yoy, usrec_df, settings, suffix=" YoY")
        else:
            self._render_lines(prod_df, usrec_df, settings)

    def _render_lines(self, df, usrec_df, settings, suffix=""):
        import pandas as pd

        if df is None or df.empty:
            self.show_placeholder("No energy production data available.")
            return
