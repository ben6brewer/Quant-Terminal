"""Money Velocity Chart — M2 velocity (quarterly ratio) with recession shading."""
...
class MoneyVelocityChart(BaseChart):
    """M2 velocity ratio (quarterly) with optional recession shading."""
    ...
    def __init__(self, parent=None):
        ...
        self._vel_values: Optional[np.ndarray] = None
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading M2 velocity data...", self)
    ...
    def update_data(
        ...
        if velocity_df is None or velocity_df.empty:
            self.show_placeholder("No M2 velocity data available.")
            return
        ...
        vel_col = "M2 Velocity" if "M2 Velocity" in velocity_df.columns else velocity_df.columns[0]
        vel_series = velocity_df[vel_col].dropna()
        if vel_series.empty:
            self.show_placeholder("No M2 velocity data available.")
            return

        self._placeholder.setVisible(False)
        self._dates = vel_series.index.values
        self._date_labels = [pd.Timestamp(d).strftime("Q%q %Y") for d in self._dates]
        self._vel_values = vel_series.values.astype(float)
        ...
        self._vel_line = self.plot_item.plot(
            x, self._vel_values,
            pen=pg.mkPen(color=accent, width=2.5)
        )
        self._vel_line.setClipToView(True)

        valid = self._vel_values[~np.isnan(self._vel_values)]
        if len(valid) > 0:
    ...
    def _update_tooltip(self, idx: int, mouse_pos):
        ...
        if self._vel_values is not None and idx < len(self._vel_values):
            val = self._vel_values[idx]
            if not np.isnan(val):
                lines.append(
                    f'<span style="color:rgb({accent[0]},{accent[1]},{accent[2]});">\u25a0</span>'
                    f" M2 Velocity: {val:.2f}"
                )
    ...
    def set_theme(self, theme: str):
        ...
        if self._vel_line is not None:
            accent = self._get_theme_accent_color()
            self._vel_line.setPen(pg.mkPen(color=accent, width=2.5))
        self._placeholder.setStyleSheet("font-size: 16px; color: #888888; background: transparent;")
