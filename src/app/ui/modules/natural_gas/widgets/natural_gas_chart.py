"""Natural Gas Chart — Dual-mode: Raw line or YoY% line."""
...
NATGAS_COLOR = "#FF7043"
SERIES_NAME = "Natural Gas"


class NaturalGasChart(BaseChart):
    """Single-line natural gas chart — Raw or YoY%."""
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading natural gas data...", self)
    ...
    def update_data(self, energy_df: "Optional[pd.DataFrame]",
        ...
        if self._view_mode == "YoY %":
            self._render_yoy(energy_df, usrec_df, settings)
        else:
            self._render_raw(energy_df, usrec_df, settings)

    # -- Raw: single line ------------------------------------------------

    def _render_raw(self, energy_df, usrec_df, settings):
        import pandas as pd

        if energy_df is None or energy_df.empty or SERIES_NAME not in energy_df.columns:
            self.show_placeholder("No natural gas data available.")
            return

        show_recession = settings.get("show_recession_bands", True)

        s = energy_df[SERIES_NAME].dropna()
        if s.empty:
            self.show_placeholder("No natural gas data available.")
            return
        ...
        self._series_values[SERIES_NAME] = vals
        line = self.plot_item.plot(x, vals, pen=pg.mkPen(color=NATGAS_COLOR, width=2), name=SERIES_NAME)
        line.setClipToView(True)
        self._line_items[SERIES_NAME] = line
    ...
    def _render_yoy(self, energy_df, usrec_df, settings):
        import pandas as pd
        ...
        yoy = energy_df[SERIES_NAME].pct_change(periods=252) * 100
        yoy = yoy.dropna()
        if yoy.empty:
    ...
    def _update_tooltip(self, idx: int, mouse_pos):
        ...
        vals = self._series_values.get(SERIES_NAME)
        if vals is not None and idx < len(vals):
            v = vals[idx]
            if not np.isnan(v):
                if self._view_mode == "YoY %":
                    lines.append(f'<span style="color:{NATGAS_COLOR};">\u25a0</span> {SERIES_NAME}: {v:+.1f}%')
                else:
                    lines.append(f'<span style="color:{NATGAS_COLOR};">\u25a0</span> {SERIES_NAME}: ${v:.2f}/MMBtu')
    ...
    def set_theme(self, theme: str):
        ...
        for name, line in self._line_items.items():
            line.setPen(pg.mkPen(color=NATGAS_COLOR, width=2))
        self._placeholder.setStyleSheet("font-size: 16px; color: #888888; background: transparent;")
