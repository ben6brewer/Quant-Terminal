"""Savings Rate Chart — Single Savings Rate (%) line with percentage Y-axis."""
...
class SavingsRateChart(BaseChart):
    """Savings Rate chart — single % line with recession shading."""
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading savings rate data...", self)
    ...
    def update_data(
        ...
        self._values = s.values.astype(float)
        ...
        self._line = self.plot_item.plot(
            x, self._values, pen=pg.mkPen(color=accent, width=2.5)
        )
        self._line.setClipToView(True)

        valid = self._values[~np.isnan(self._values)]
        if len(valid) > 0:
    ...
    def _update_tooltip(self, idx: int, mouse_pos):
        ...
        if self._values is not None and idx < len(self._values):
            val = self._values[idx]
            if not np.isnan(val):
                color_str = f"rgb({accent[0]},{accent[1]},{accent[2]})"
                lines.append(
                    f'<span style="color:{color_str};">\u25a0</span>'
                    f" Savings Rate: {val:.1f}%"
                )
    ...
    def set_theme(self, theme: str):
        ...
        if self._line is not None:
            accent = self._get_theme_accent_color()
            self._line.setPen(pg.mkPen(color=accent, width=2.5))
        self._placeholder.setStyleSheet("font-size: 16px; color: #888888; background: transparent;")
