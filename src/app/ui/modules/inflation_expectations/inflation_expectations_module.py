...
from .services.inflation_expectations_settings_manager import InflationExpectationsSettingsManager
...
LOOKBACK_MONTHS = {
    "1Y": 12, "2Y": 24, "5Y": 60, "10Y": 120, "Max": None,
}


class InflationExpectationsModule(BaseModule):
    """Inflation Expectations — 5Y/10Y breakevens + Michigan 1Y survey from FRED."""

    def __init__(self, theme_manager: ThemeManager, parent=None):
        super().__init__(theme_manager, parent)

        self.settings_manager = InflationExpectationsSettingsManager()
    ...
    def _setup_ui(self):
        ...
        self.toolbar = InflationExpectationsToolbar(self.theme_manager)
        layout.addWidget(self.toolbar)

        self.chart = InflationExpectationsChart()
    ...
    def _fetch_data(self):
        self._run_worker(
            ...
            loading_message="Fetching inflation expectations from FRED...",
    ...
    def _on_data_fetched(self, result):
        ...
        if result is None:
            self.chart.show_placeholder("Failed to fetch inflation expectations data.")
            return
        self._data = result
    ...
    def _render(self):
        ...
        exp_df = self._slice_monthly(self._data.get("expectations"))
        self.chart.update_data(exp_df, settings)
    ...
    def _on_settings_clicked(self):
        ...
        class InflationExpectationsSettingsDialog(ThemedDialog):
            def __init__(self, theme_manager, current_settings, parent=None):
                self._current = current_settings
                self._checks = {}
                super().__init__(theme_manager, "Inflation Expectations Settings", parent, min_width=360)
        ...
        dialog = InflationExpectationsSettingsDialog(
            self.theme_manager,
