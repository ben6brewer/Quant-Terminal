"""Bank Lending Chart — Dual-mode: Raw stacked area ($B) or YoY% multi-line."""
...
# Stacked area components (bottom to top)
SERIES_ORDER: List[str] = ["C&I Loans", "Real Estate", "Consumer"]

# All 4 series for YoY% multi-line view
YOY_SERIES_ORDER: List[str] = ["Total Loans", "C&I Loans", "Real Estate", "Consumer"]
...
SERIES_VISIBILITY_KEYS: Dict[str, str] = {
    "C&I Loans": "show_ci",
    "Real Estate": "show_real_estate",
    "Consumer": "show_consumer",
}


class BankLendingChart(BaseChart):
    """Bank lending chart — Raw stacked area ($B) or YoY% multi-line."""
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading bank lending data...", self)
    ...
    def update_data(self, loans_df: "Optional[pd.DataFrame]",
        ...
        if self._view_mode == "YoY %":
            self._render_yoy(loans_df, usrec_df, settings)
        else:
            self._render_raw(loans_df, usrec_df, settings)

    # -- Raw: stacked area ($B) ------------------------------------------------

    def _render_raw(self, loans_df, usrec_df, settings):
        import pandas as pd

        if loans_df is None or loans_df.empty:
            self.show_placeholder("No bank lending data available.")
            return
        ...
        ref_series = None
        for name in visible:
            if name in loans_df.columns:
                s = loans_df[name].dropna()
                if not s.empty:
                    ref_series = s
                    break
        ...
        for label in SERIES_ORDER:
            if label not in visible or label not in loans_df.columns:
                continue
            vals = loans_df[label].reindex(ref_series.index).values.astype(float)
            safe_vals = np.nan_to_num(vals, nan=0.0)
            self._series_values[label] = vals

            color = QColor(AREA_COLORS[label])
        ...
        # Store total (stack sum) for tooltip
        self._series_values["Total"] = cumsum.copy()

        if len(cumsum) > 0:
            y_max = float(np.nanmax(cumsum))
            pad = y_max * 0.08 if y_max > 0 else 1.0
    ...
    # -- YoY %: pct_change on weekly data (periods=52) -----------------------

    def _render_yoy(self, loans_df, usrec_df, settings):
        import pandas as pd
        ...
        # YoY% uses all 4 series including Total
        yoy_visibility = {
            "Total Loans": True,  # always show total in YoY%
            "C&I Loans": settings.get("show_ci", True),
            "Real Estate": settings.get("show_real_estate", True),
            "Consumer": settings.get("show_consumer", True),
        }
        visible = [n for n in YOY_SERIES_ORDER if yoy_visibility.get(n, True)]
        if not visible:
            self.show_placeholder("No series selected.")
            return
        ...
        for col in visible:
            if col in loans_df.columns:
                yoy = loans_df[col].pct_change(periods=52) * 100
                yoy_data[col] = yoy
        ...
        for name in YOY_SERIES_ORDER:
            if name not in visible or name not in yoy_df.columns:
                continue
            vals = yoy_df[name].values.astype(float)
            self._series_values[name] = vals
            color = accent if name == "Total Loans" else AREA_COLORS.get(name, "#888888")
        ...
        if self._view_mode == "YoY %":
            for name in YOY_SERIES_ORDER:
                ...
                if name == "Total Loans":
                    ...
                    color_str = AREA_COLORS.get(name, "#888888")
                lines.append(f'<span style="color:{color_str};">\u25a0</span> {name}: {v:+.1f}%')
        else:
            # Raw stacked area — show each component + total
            for name in SERIES_ORDER:
                ...
                if np.isnan(v):
                    continue
                color_str = AREA_COLORS.get(name, "#888888")
                lines.append(f'<span style="color:{color_str};">\u25a0</span> {name}: ${v:,.0f}B')
            total_vals = self._series_values.get("Total")
            if total_vals is not None and idx < len(total_vals):
                t = total_vals[idx]
                if not np.isnan(t):
                    lines.append(f"<b>Total: ${t:,.0f}B</b>")
    ...
    def set_theme(self, theme: str):
        ...
        for name, line in self._line_items.items():
            if name == "Total Loans":
                ...
                line.setPen(pg.mkPen(color=AREA_COLORS.get(name, "#888888"), width=2))
        self._placeholder.setStyleSheet("font-size: 16px; color: #888888; background: transparent;")
