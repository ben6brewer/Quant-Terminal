"""Vehicle Sales Chart — Multi-line: Total, Light Autos, Heavy Trucks."""
...
VEHICLE_SERIES_ORDER: List[str] = ["Total Vehicle Sales", "Light Autos", "Heavy Trucks"]

SERIES_VISIBILITY_KEYS: Dict[str, str] = {
    "Total Vehicle Sales": "show_total",
    "Light Autos": "show_light_autos",
    "Heavy Trucks": "show_heavy_trucks",
}


class VehicleSalesChart(BaseChart):
    """Multi-line vehicle sales chart."""
    ...
    def update_data(self, vehicles_df: "Optional[pd.DataFrame]",
        ...
        visible = [n for n in VEHICLE_SERIES_ORDER
        ...
        all_vals = []
        for name in VEHICLE_SERIES_ORDER:
            if name not in visible or name not in vehicles_df.columns:
                continue
            vals = vehicles_df[name].reindex(ref_series.index).values.astype(float)
            self._series_values[name] = vals
            color = accent if name == "Total Vehicle Sales" else VEHICLE_COLORS.get(name, "#888888")
            style = Qt.SolidLine if name == "Total Vehicle Sales" else Qt.DashLine
            line = self.plot_item.plot(
                x, vals, pen=pg.mkPen(color=color, width=2, style=style), name=name
            )
    ...
    def _update_tooltip(self, idx: int, mouse_pos):
        ...
        for name in VEHICLE_SERIES_ORDER:
            ...
            if np.isnan(v):
                continue
            if name == "Total Vehicle Sales":
                color_str = f"rgb({accent[0]},{accent[1]},{accent[2]})"
            else:
                color_str = VEHICLE_COLORS.get(name, "#888888")
            lines.append(f'<span style="color:{color_str};">\u25a0</span> {name}: {v:.1f}M')
    ...
    def set_theme(self, theme: str):
        ...
        for name, line in self._line_items.items():
            if name == "Total Vehicle Sales":
                ...
                color = VEHICLE_COLORS.get(name, "#888888")
                line.setPen(pg.mkPen(color=color, width=2, style=Qt.DashLine))
        self._placeholder.setStyleSheet("font-size: 16px; color: #888888; background: transparent;")
