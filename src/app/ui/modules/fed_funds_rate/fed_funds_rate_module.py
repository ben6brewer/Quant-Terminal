...
from .services.fed_funds_rate_settings_manager import FedFundsRateSettingsManager
...
class FedFundsRateModule(BaseModule):
    """Fed Funds Rate module — full EFFR history from FRED."""

    def __init__(self, theme_manager: ThemeManager, parent=None):
        super().__init__(theme_manager, parent)

        self.settings_manager = FedFundsRateSettingsManager()
    ...
    def _setup_ui(self):
        ...
        self.toolbar = FedFundsRateToolbar(self.theme_manager)
        layout.addWidget(self.toolbar)

        self.chart = FedFundsRateChart()
    ...
    def _fetch_data(self):
        self._run_worker(
            ...
            loading_message="Fetching Fed funds rate data from FRED...",
    ...
    def _on_data_fetched(self, result):
        ...
        if result is None:
            self.chart.show_placeholder("Failed to fetch Fed funds rate data.")
            return
        self._data = result
    ...
    def _render(self):
        ...
        effr_df = self._slice_data(self._data.get("effr"))
        usrec_df = self._data.get("usrec")
        self.chart.update_data(effr_df, usrec_df, settings)
    ...
    def _on_settings_clicked(self):
        ...
        class FedFundsSettingsDialog(ThemedDialog):
            def __init__(self, theme_manager, current_settings, parent=None):
                self._current = current_settings
                self._checks = {}
                super().__init__(theme_manager, "Fed Funds Rate Settings", parent, min_width=360)
