"""Corporate Spreads Chart — Multi-line: Baa-10Y, Aaa-10Y, HY OAS."""
...
SPREAD_COLORS: Dict[str, str] = {
    "Aaa-10Y Spread": "#66BB6A",
    "HY OAS": "#FF7043",
}

SPREAD_SERIES_ORDER: List[str] = ["Baa-10Y Spread", "Aaa-10Y Spread", "HY OAS"]

SERIES_VISIBILITY_KEYS: Dict[str, str] = {
    "Baa-10Y Spread": "show_baa",
    "Aaa-10Y Spread": "show_aaa",
    "HY OAS": "show_hy",
}


class CorporateSpreadsChart(BaseChart):
    """Multi-line corporate spreads chart."""
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading corporate spread data...", self)
    ...
    def update_data(self, spreads_df: "Optional[pd.DataFrame]",
        ...
        if spreads_df is None or spreads_df.empty:
            self.show_placeholder("No corporate spread data available.")
            return

        show_recession = settings.get("show_recession_bands", True)

        visible = [n for n in SPREAD_SERIES_ORDER
        ...
        ref_series = None
        for name in visible:
            if name in spreads_df.columns:
                s = spreads_df[name].dropna()
                if not s.empty:
                    ref_series = s
                    break
        ...
        all_vals = []
        for name in SPREAD_SERIES_ORDER:
            if name not in visible or name not in spreads_df.columns:
                continue
            vals = spreads_df[name].reindex(ref_series.index).values.astype(float)
            self._series_values[name] = vals
            color = accent if name == "Baa-10Y Spread" else SPREAD_COLORS.get(name, "#888888")
    ...
    def _update_tooltip(self, idx: int, mouse_pos):
        ...
        for name in SPREAD_SERIES_ORDER:
            ...
            if np.isnan(v):
                continue
            if name == "Baa-10Y Spread":
                color_str = f"rgb({accent[0]},{accent[1]},{accent[2]})"
            else:
                color_str = SPREAD_COLORS.get(name, "#888888")
            unit = "bp" if name == "HY OAS" else "%"
            fmt_v = f"{v:.0f}" if name == "HY OAS" else f"{v:.2f}"
            lines.append(f'<span style="color:{color_str};">\u25a0</span> {name}: {fmt_v}{unit}')
    ...
    def set_theme(self, theme: str):
        ...
        for name, line in self._line_items.items():
            if name == "Baa-10Y Spread":
                ...
                line.setPen(pg.mkPen(color=SPREAD_COLORS.get(name, "#888888"), width=2))
        self._placeholder.setStyleSheet("font-size: 16px; color: #888888; background: transparent;")
