"""Rolling Settings Dialog - Settings for rolling correlation/covariance chart."""
...
class RollingSettingsDialog(ThemedDialog):
    """Settings dialog for Rolling Correlation and Rolling Covariance modules."""
    ...
    def __init__(self, theme_manager, current_settings: dict, mode: str = "correlation", parent=None):
        self._current_settings = current_settings
        self._mode = mode
        self._line_color = current_settings.get("line_color", None)
        self.result = None
        title = "Rolling Correlation Settings" if mode == "correlation" else "Rolling Covariance Settings"
        super().__init__(theme_manager, title, parent, min_width=420)
    ...
    def _setup_content(self, layout):
        ...
        idx = self.line_style_combo.findText(style_name)
        if idx >= 0:
