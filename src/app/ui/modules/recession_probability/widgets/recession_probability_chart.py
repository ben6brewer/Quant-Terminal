"""Recession Probability Chart — Single-line RECPROUSM156N."""
...
class RecessionProbabilityChart(BaseChart):
    """Single-line recession probability chart."""
    ...
    def __init__(self, parent=None):
        ...
        self._prob_values: Optional[np.ndarray] = None
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading recession probability data...", self)
    ...
    def update_data(self, prob_df: "Optional[pd.DataFrame]",
        ...
        if prob_df is None or prob_df.empty:
            self.show_placeholder("No recession probability data available.")
            return

        show_recession = settings.get("show_recession_bands", True)

        col = "Recession Prob" if "Recession Prob" in prob_df.columns else prob_df.columns[0]
        series = prob_df[col].dropna()
        if series.empty:
            self.show_placeholder("No recession probability data available.")
            return
        ...
        self._prob_values = series.values.astype(float)
        ...
        self._prob_line = self.plot_item.plot(
            x, self._prob_values,
            pen=pg.mkPen(color=accent, width=2.5)
        )
        self._prob_line.setClipToView(True)

        valid = self._prob_values[~np.isnan(self._prob_values)]
        if len(valid) > 0:
    ...
    def _update_tooltip(self, idx: int, mouse_pos):
        ...
        if self._prob_values is not None and idx < len(self._prob_values):
            val = self._prob_values[idx]
            if not np.isnan(val):
                lines.append(
    ...
    def set_theme(self, theme: str):
        ...
        self._apply_tooltip_style()
        if self._prob_line is not None:
            accent = self._get_theme_accent_color()
            self._prob_line.setPen(pg.mkPen(color=accent, width=2.5))
        self._placeholder.setStyleSheet("font-size: 16px; color: #888888; background: transparent;")
