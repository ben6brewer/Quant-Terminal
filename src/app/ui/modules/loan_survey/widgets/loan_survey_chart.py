"""Loan Survey (SLOOS) Chart — Lending standards across categories."""
...
_SERIES_COLORS = {
    "C&I Large Firms": "#4FC3F7",
    "C&I Small Firms": "#81C784",
    "Credit Cards": "#FFA726",
    "Mortgages": "#CE93D8",
    "Auto Loans": "#EF5350",
}
...
_SERIES_SETTINGS = {
    "C&I Large Firms": "show_ci_large",
    "C&I Small Firms": "show_ci_small",
    "Credit Cards": "show_credit_cards",
    "Mortgages": "show_mortgages",
    "Auto Loans": "show_auto_loans",
}


class LoanSurveyChart(BaseChart):
    """SLOOS lending standards chart with zero reference line."""
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading SLOOS data...", self)
    ...
    def update_data(self, standards_df: "Optional[pd.DataFrame]",
        ...
        self._view_mode = settings.get("view_mode", "All Standards")

        if self._view_mode == "By Category":
            # Show only C&I series
            if standards_df is not None and not standards_df.empty:
                ci_cols = [c for c in ["C&I Large Firms", "C&I Small Firms"] if c in standards_df.columns]
                if ci_cols:
                    self._render_lines(standards_df[ci_cols], usrec_df, settings)
                    return
        self._render_lines(standards_df, usrec_df, settings)

    def _render_lines(self, df, usrec_df, settings, suffix=""):
        import pandas as pd

        if df is None or df.empty:
            self.show_placeholder("No SLOOS data available.")
            return
    ...
    def _update_tooltip(self, idx: int, mouse_pos):
        ...
        for name, vals in self._series_values.items():
            ...
            if np.isnan(v):
                continue
            base_name = name.replace(" YoY", "").replace(" Growth", "")
