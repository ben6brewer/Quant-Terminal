"""Financial Stress Chart — Multi-line: STLFSI + KCFSI with zero reference."""
...
SERIES_ORDER: List[str] = ["STLFSI", "KCFSI"]
...
class FinancialStressChart(BaseChart):
    """Multi-line financial stress chart with zero reference line."""
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading financial stress data...", self)
    ...
    def update_data(self, stress_df: "Optional[pd.DataFrame]",
        ...
        if stress_df is None or stress_df.empty:
            self.show_placeholder("No financial stress data available.")
            return
        ...
        ref_series = None
        for name in visible:
            if name in stress_df.columns:
                s = stress_df[name].dropna()
                if not s.empty:
                    ref_series = s
                    break
        ...
        for name in SERIES_ORDER:
            if name not in visible or name not in stress_df.columns:
                continue
            vals = stress_df[name].reindex(ref_series.index).values.astype(float)
            self._series_values[name] = vals
            color = accent if name == "STLFSI" else SERIES_COLORS.get(name, "#888888")
