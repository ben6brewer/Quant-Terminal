"""Financial Conditions Chart — Multi-line NFCI + subindices with zero ref."""
...
NFCI_COLORS: Dict[str, str] = {
    "Adjusted NFCI": "#66BB6A",
    "Credit Sub": "#EF5350",
    "Leverage Sub": "#FF7043",
    "NonFin Leverage Sub": "#4FC3F7",
}

NFCI_SERIES_ORDER: List[str] = [
    "NFCI", "Adjusted NFCI", "Credit Sub", "Leverage Sub", "NonFin Leverage Sub"
]

SERIES_VISIBILITY_KEYS: Dict[str, str] = {
    "NFCI": "show_nfci",
    "Adjusted NFCI": "show_adjusted",
    "Credit Sub": "show_credit",
    "Leverage Sub": "show_leverage",
    "NonFin Leverage Sub": "show_nonfin_leverage",
}


class FinancialConditionsChart(BaseChart):
    """Multi-line NFCI chart with zero reference line."""
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading financial conditions data...", self)
    ...
    def update_data(self, nfci_df: "Optional[pd.DataFrame]",
        ...
        if nfci_df is None or nfci_df.empty:
            self.show_placeholder("No financial conditions data available.")
            return

        show_recession = settings.get("show_recession_bands", True)

        visible = [n for n in NFCI_SERIES_ORDER
        ...
        ref_series = None
        for name in visible:
            if name in nfci_df.columns:
                s = nfci_df[name].dropna()
                if not s.empty:
                    ref_series = s
                    break
        ...
        all_vals = []
        for name in NFCI_SERIES_ORDER:
            if name not in visible or name not in nfci_df.columns:
                continue
            vals = nfci_df[name].reindex(ref_series.index).values.astype(float)
            self._series_values[name] = vals
            color = accent if name == "NFCI" else NFCI_COLORS.get(name, "#888888")
    ...
    def _update_tooltip(self, idx: int, mouse_pos):
        ...
        for name in NFCI_SERIES_ORDER:
            ...
            if name == "NFCI":
                ...
                color_str = NFCI_COLORS.get(name, "#888888")
            lines.append(f'<span style="color:{color_str};">\u25a0</span> {name}: {v:+.3f}')
    ...
    def set_theme(self, theme: str):
        ...
        for name, line in self._line_items.items():
            if name == "NFCI":
                ...
                line.setPen(pg.mkPen(color=NFCI_COLORS.get(name, "#888888"), width=2))
        self._placeholder.setStyleSheet("font-size: 16px; color: #888888; background: transparent;")
