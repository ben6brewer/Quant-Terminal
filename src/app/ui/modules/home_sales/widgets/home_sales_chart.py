"""Home Sales Chart — Existing/new home sales with supply months."""
...
_SERIES_COLORS = {
    "Existing Home Sales": "#4FC3F7",
    "New Home Sales": "#81C784",
}

_SERIES_SETTINGS = {
    "Existing Home Sales": "show_existing_sales",
    "New Home Sales": "show_new_sales",
}


class HomeSalesChart(BaseChart):
    """Home sales multi-line chart with Raw/YoY% modes."""
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading home sales data...", self)
    ...
    def update_data(self, sales_df: "Optional[pd.DataFrame]",
        ...
        if self._view_mode == "YoY %" and sales_df is not None and not sales_df.empty:
            yoy = sales_df.pct_change(periods=12) * 100
            yoy = yoy.dropna(how="all")
            self._render_lines(yoy, usrec_df, settings, suffix=" YoY")
        else:
            self._render_lines(sales_df, usrec_df, settings)

    def _render_lines(self, df, usrec_df, settings, suffix=""):
        import pandas as pd

        if df is None or df.empty:
            self.show_placeholder("No home sales data available.")
            return
    ...
    def _update_tooltip(self, idx: int, mouse_pos):
        ...
        for name, vals in self._series_values.items():
            ...
            if "YoY" in name or "Growth" in name:
                ...
                lines.append(f'<span style="color:{color_str};">\u25a0</span> {name}: {v:,.0f}K')
