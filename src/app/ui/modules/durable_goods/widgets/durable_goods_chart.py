"""Durable Goods Chart — Dual-mode: Raw multi-line or YoY% growth."""
...
DG_COLORS: Dict[str, str] = {
    "Core Capital Goods": "#66BB6A",
    "Unfilled Orders": "#FF7043",
}

DG_SERIES_ORDER: List[str] = ["Durable Goods", "Core Capital Goods", "Unfilled Orders"]


class DurableGoodsChart(BaseChart):
    """Durable goods chart — multi-line (Raw) or YoY% lines."""
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading durable goods data...", self)
    ...
    def update_data(self, orders_df: "Optional[pd.DataFrame]",
        ...
        if self._view_mode == "YoY %":
            self._render_yoy(orders_df, usrec_df, settings)
        else:
            self._render_raw(orders_df, usrec_df, settings)

    def _render_raw(self, orders_df, usrec_df, settings):
        import pandas as pd

        if orders_df is None or orders_df.empty:
            self.show_placeholder("No durable goods data available.")
            return

        show_recession = settings.get("show_recession_bands", True)
        ref_series = None
        for name in DG_SERIES_ORDER:
            if name in orders_df.columns:
                s = orders_df[name].dropna()
                if not s.empty:
                    ref_series = s
                    break
        ...
        for name in DG_SERIES_ORDER:
            if name not in orders_df.columns:
                continue
            vals = orders_df[name].reindex(ref_series.index).values.astype(float)
            self._series_values[name] = vals
            color = accent if name == "Durable Goods" else DG_COLORS.get(name, "#888888")
    ...
    def _render_yoy(self, orders_df, usrec_df, settings):
        import pandas as pd
        ...
        for name in DG_SERIES_ORDER:
            if name in orders_df.columns:
                yoy = orders_df[name].pct_change(periods=12) * 100
                yoy_frames[name + " YoY%"] = yoy

        if not yoy_frames:
            self.show_placeholder("No durable goods data available.")
            return

        yoy_df = pd.DataFrame(yoy_frames).dropna(how="all")
        if yoy_df.empty:
            self.show_placeholder("Not enough data for YoY% calculation.")
            return

        ref_col = next(iter(yoy_df.columns))
        ref = yoy_df[ref_col].dropna()
        if ref.empty:
            self.show_placeholder("Not enough data for YoY% calculation.")
            return

        self._placeholder.setVisible(False)
        self._dates = ref.index.values
        self._date_labels = [pd.Timestamp(d).strftime("%b %Y") for d in self._dates]
        ...
        all_vals = []
        for orig_name in DG_SERIES_ORDER:
            col_name = orig_name + " YoY%"
            if col_name not in yoy_df.columns:
                continue
            vals = yoy_df[col_name].reindex(ref.index).values.astype(float)
            self._series_values[col_name] = vals
            color = accent if orig_name == "Durable Goods" else DG_COLORS.get(orig_name, "#888888")
            line = self.plot_item.plot(x, vals, pen=pg.mkPen(color=color, width=2), name=col_name)
            line.setClipToView(True)
            self._line_items[col_name] = line
            valid = vals[~np.isnan(vals)]
            if len(valid) > 0:
                all_vals.extend(valid.tolist())
        ...
        if all_vals:
            y_min, y_max = min(all_vals), max(all_vals)
            pad = (y_max - y_min) * 0.1 if y_max != y_min else 2.0
            self.plot_item.setYRange(y_min - pad, y_max + pad, padding=0)
    ...
    def _update_tooltip(self, idx: int, mouse_pos):
        ...
        for name, vals in self._series_values.items():
            ...
            if np.isnan(v):
                continue
            is_primary = "Durable Goods" in name and not any(
                k in name for k in ["Core", "Unfilled"])
            if is_primary:
                color_str = f"rgb({accent[0]},{accent[1]},{accent[2]})"
            else:
                base = name.replace(" YoY%", "")
                color_str = DG_COLORS.get(base, "#888888")
            if "YoY%" in name:
    ...
    def set_theme(self, theme: str):
        ...
        for name, line in self._line_items.items():
            ...
            if is_primary:
                ...
                base = name.replace(" YoY%", "")
                line.setPen(pg.mkPen(color=DG_COLORS.get(base, "#888888"), width=2))
        self._placeholder.setStyleSheet("font-size: 16px; color: #888888; background: transparent;")
