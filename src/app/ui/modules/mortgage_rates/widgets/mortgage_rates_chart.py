"""Mortgage Rates Chart — Multi-line: 30Y and 15Y fixed rates."""
...
RATE_COLORS: Dict[str, str] = {
    "15Y Fixed": "#66BB6A",
}

RATE_SERIES_ORDER: List[str] = ["30Y Fixed", "15Y Fixed"]


class MortgageRatesChart(BaseChart):
    """Multi-line mortgage rates chart."""
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading mortgage rate data...", self)
    ...
    def update_data(self, rates_df: "Optional[pd.DataFrame]",
        ...
        if rates_df is None or rates_df.empty:
            self.show_placeholder("No mortgage rate data available.")
            return

        show_recession = settings.get("show_recession_bands", True)

        ref_series = None
        for name in RATE_SERIES_ORDER:
            if name in rates_df.columns:
                s = rates_df[name].dropna()
                if not s.empty:
                    ref_series = s
                    break
        ...
        all_vals = []
        for name in RATE_SERIES_ORDER:
            if name not in rates_df.columns:
                continue
            vals = rates_df[name].reindex(ref_series.index).values.astype(float)
            self._series_values[name] = vals
            color = accent if name == "30Y Fixed" else RATE_COLORS.get(name, "#888888")
    ...
    def _update_tooltip(self, idx: int, mouse_pos):
        ...
        for name in RATE_SERIES_ORDER:
            ...
            if name == "30Y Fixed":
                ...
                color_str = RATE_COLORS.get(name, "#888888")
    ...
    def set_theme(self, theme: str):
        ...
        for name, line in self._line_items.items():
            if name == "30Y Fixed":
                ...
                line.setPen(pg.mkPen(color=RATE_COLORS.get(name, "#888888"), width=2))
        self._placeholder.setStyleSheet("font-size: 16px; color: #888888; background: transparent;")
