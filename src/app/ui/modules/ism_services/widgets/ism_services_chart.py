"""ISM Services Chart — Single-line: Services Activity with 50 threshold."""
...
SERIES_ORDER: List[str] = ["Services Activity"]


class IsmServicesChart(BaseChart):
    """Single-line ISM Services chart with 50 threshold line."""
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading ISM Services data...", self)
    ...
    def update_data(self, svc_df: "Optional[pd.DataFrame]",
        ...
        if svc_df is None or svc_df.empty:
            self.show_placeholder("No ISM Services data available.")
            return

        show_recession = settings.get("show_recession_bands", True)

        col = "Services Activity" if "Services Activity" in svc_df.columns else svc_df.columns[0]
        series = svc_df[col].dropna()
        if series.empty:
            self.show_placeholder("No ISM Services data available.")
            return
        ...
        # Plot Services Activity line
        vals = series.values.astype(float)
        self._series_values["Services Activity"] = vals
        line = self.plot_item.plot(
            x, vals, pen=pg.mkPen(color=accent, width=2.5), name="Services Activity"
        )
        line.setClipToView(True)
        self._line_items["Services Activity"] = line
        ...
        if len(valid) > 0:
            ...
            y_min = min(y_min, 50)
            y_max = max(y_max, 50)
            y_range = y_max - y_min if y_max != y_min else 5.0
            pad = y_range * 0.08
    ...
    def _update_tooltip(self, idx: int, mouse_pos):
        ...
        vals = self._series_values.get("Services Activity")
        if vals is not None and idx < len(vals):
            v = vals[idx]
            if not np.isnan(v):
                color_str = f"rgb({accent[0]},{accent[1]},{accent[2]})"
                lines.append(
                    f'<span style="color:{color_str};">\u25a0</span> Services Activity: {v:.1f}'
                )
    ...
    def set_theme(self, theme: str):
        ...
        self._apply_tooltip_style()
        if "Services Activity" in self._line_items:
            accent = self._get_theme_accent_color()
            self._line_items["Services Activity"].setPen(pg.mkPen(color=accent, width=2.5))
        self._placeholder.setStyleSheet("font-size: 16px; color: #888888; background: transparent;")
