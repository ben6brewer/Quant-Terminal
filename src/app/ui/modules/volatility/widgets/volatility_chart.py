"""Volatility Chart — Multi-line: VIX, 3M Vol, Oil Vol with threshold lines."""
...
SERIES_ORDER: List[str] = [
    "VIX", "NASDAQ Vol", "MOVE", "3M Vol", "Oil Vol",
    "Russell Vol", "DJIA Vol", "EM Vol",
]
...
SERIES_VISIBILITY_KEYS: Dict[str, str] = {
    "VIX": "show_vix",
    "3M Vol": "show_3m_vol",
    "Oil Vol": "show_oil_vol",
    "NASDAQ Vol": "show_nasdaq_vol",
    "Russell Vol": "show_russell_vol",
    "DJIA Vol": "show_djia_vol",
    "EM Vol": "show_em_vol",
    "MOVE": "show_move",
}

# Series that render with dashed lines
_DASHED_SERIES = {"Oil Vol", "MOVE"}


class VolatilityChart(BaseChart):
    """Multi-line volatility chart with threshold lines."""
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading volatility data...", self)
    ...
    def update_data(self, vol_df: "Optional[pd.DataFrame]",
        ...
        if vol_df is None or vol_df.empty:
            self.show_placeholder("No volatility data available.")
            return
        ...
        ref_series = None
        for name in visible:
            if name in vol_df.columns:
                s = vol_df[name].dropna()
                if not s.empty:
                    ref_series = s
                    break
        ...
        show_thresholds = settings.get("show_thresholds", True)
        threshold_1 = settings.get("threshold_1", 20)
        threshold_2 = settings.get("threshold_2", 30)

        if show_thresholds:
            line_1 = pg.InfiniteLine(
                pos=threshold_1, angle=0, pen=pg.mkPen(color="#888888", width=1, style=Qt.DashLine)
            )
            self.plot_item.addItem(line_1)
            self._threshold_lines.append(line_1)

            line_2 = pg.InfiniteLine(
                pos=threshold_2, angle=0, pen=pg.mkPen(color="#FF6B6B", width=1, style=Qt.DashLine)
            )
            self.plot_item.addItem(line_2)
            self._threshold_lines.append(line_2)

        all_vals = []
        for name in SERIES_ORDER:
            if name not in visible or name not in vol_df.columns:
                continue
            vals = vol_df[name].reindex(ref_series.index).values.astype(float)
            self._series_values[name] = vals
            color = accent if name == "VIX" else SERIES_COLORS.get(name, "#888888")
            width = 2 if name in ("VIX", "MOVE") else 1
            style = Qt.DashLine if name in _DASHED_SERIES else Qt.SolidLine
            line = self.plot_item.plot(
                x, vals, pen=pg.mkPen(color=color, width=width, style=style), name=name
            )
        ...
        if all_vals:
            ...
            if show_thresholds:
                y_min = min(y_min, threshold_1, threshold_2)
                y_max = max(y_max, threshold_1, threshold_2)
    ...
    def set_theme(self, theme: str):
        ...
        for name, line in self._line_items.items():
            ...
            elif name in _DASHED_SERIES:
                w = 2 if name == "MOVE" else 1
                line.setPen(pg.mkPen(
                    color=SERIES_COLORS.get(name, "#888888"), width=w, style=Qt.DashLine
                ))
            else:
                line.setPen(pg.mkPen(color=SERIES_COLORS.get(name, "#888888"), width=1))
        self._placeholder.setStyleSheet("font-size: 16px; color: #888888; background: transparent;")
