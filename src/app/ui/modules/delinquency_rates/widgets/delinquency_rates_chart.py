"""Delinquency Rates Chart — Multi-line delinquency rates (%)."""
...
SERIES_ORDER: List[str] = ["Credit Card Delinquency", "All Loans Delinquency", "Consumer Loans Delinquency"]


class DelinquencyRatesChart(BaseChart):
    """Multi-line delinquency rates chart (%)."""
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading delinquency data...", self)
    ...
    def update_data(self, delinq_df: "Optional[pd.DataFrame]",
        ...
        if delinq_df is None or delinq_df.empty:
            self.show_placeholder("No delinquency data available.")
            return

        ref_series = None
        for name in SERIES_ORDER:
            if name in delinq_df.columns:
                s = delinq_df[name].dropna()
                if not s.empty:
                    ref_series = s
                    break
        ...
        all_vals = []
        for name in SERIES_ORDER:
            if name not in delinq_df.columns:
                continue
            vals = delinq_df[name].reindex(ref_series.index).values.astype(float)
