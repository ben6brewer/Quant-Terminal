"""Regional PMI Chart — Regional Fed manufacturing indices."""
...
_SERIES_SETTINGS = {
    "Empire State": "show_empire",
    "Philly Fed": "show_philly",
    "Dallas Fed": "show_dallas",
}


class RegionalPmiChart(BaseChart):
    """Regional PMI multi-line chart with zero reference line."""
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading regional PMI data...", self)
    ...
    def update_data(self, regional_df: "Optional[pd.DataFrame]",
        ...
        self._render_lines(regional_df, usrec_df, settings)

    def _render_lines(self, df, usrec_df, settings, suffix=""):
        import pandas as pd

        if df is None or df.empty:
            self.show_placeholder("No regional PMI data available.")
            return
