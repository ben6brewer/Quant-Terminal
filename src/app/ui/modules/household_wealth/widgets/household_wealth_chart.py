"""Household Wealth Chart — Dual-mode: Raw Net Worth ($T) or YoY%."""
...
class HouseholdWealthChart(BaseChart):
    """Single-line household wealth chart — Raw or YoY%."""
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading household wealth data...", self)
    ...
    def update_data(self, wealth_df: "Optional[pd.DataFrame]",
        ...
        if self._view_mode == "YoY %":
            self._render_yoy(wealth_df, usrec_df, settings)
        else:
            self._render_raw(wealth_df, usrec_df, settings)

    # -- Raw: Net Worth $T single line ----------------------------------------

    def _render_raw(self, df, usrec_df, settings):
        import pandas as pd

        if df is None or df.empty or "Net Worth" not in df.columns:
            self.show_placeholder("No household wealth data available.")
            return

        show_recession = settings.get("show_recession_bands", True)

        s = df["Net Worth"].dropna()
        if s.empty:
            self.show_placeholder("No household wealth data available.")
            return
    ...
    # -- YoY %: pct_change on quarterly data ----------------------------------

    def _render_yoy(self, df, usrec_df, settings):
        import pandas as pd
        ...
        yoy = df["Net Worth"].pct_change(periods=4) * 100
        yoy = yoy.dropna()
        if yoy.empty:
        ...
        self._series_values["Net Worth"] = yoy_vals
    ...
    def _update_tooltip(self, idx: int, mouse_pos):
        ...
        vals = self._series_values.get("Net Worth")
        if vals is not None and idx < len(vals):
            v = vals[idx]
            if not np.isnan(v):
                color_str = f"rgb({accent[0]},{accent[1]},{accent[2]})"
                if self._view_mode == "YoY %":
                    lines.append(f'<span style="color:{color_str};">\u25a0</span> Net Worth: {v:+.1f}%')
                else:
                    lines.append(f'<span style="color:{color_str};">\u25a0</span> Net Worth: ${v:.1f}T')
