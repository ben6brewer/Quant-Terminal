"""Metals Chart — Multi-line overlay: Gold, Silver, Copper, Platinum, Palladium.

Dual view: Normalized (indexed to 100) / Raw (actual $ prices).
"""
...
SERIES_ORDER: List[str] = ["Gold", "Silver", "Copper", "Platinum", "Palladium"]
...
SERIES_VISIBILITY_KEYS: Dict[str, str] = {
    ...
    "Silver": "show_silver",
    "Copper": "show_copper",
    "Platinum": "show_platinum",
    "Palladium": "show_palladium",
}


class MetalsChart(BaseChart):
    """Multi-line metals chart — Normalized or Raw."""
    ...
    def __init__(self, parent=None):
        ...
        self._raw_values: Dict[str, np.ndarray] = {}
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading metals data...", self)
    ...
    def update_data(self, metals_df: "Optional[pd.DataFrame]", settings: dict):
        ...
        self._view_mode = settings.get("view_mode", "Normalized")

        if self._view_mode == "Raw":
            self._render_raw(metals_df, settings)
        else:
            self._render_normalized(metals_df, settings)

    # -- Normalized: index to 100 at start of window -----------------------

    def _render_normalized(self, metals_df, settings):
        import pandas as pd

        if metals_df is None or metals_df.empty:
            self.show_placeholder("No metals data available.")
            return
        ...
        # Find reference series for dates
        ref_series = None
        for name in visible:
            if name in metals_df.columns:
                s = metals_df[name].dropna()
                if not s.empty:
                    ref_series = s
                    break
        ...
        for name in SERIES_ORDER:
            if name not in visible or name not in metals_df.columns:
                continue
            raw = metals_df[name].reindex(ref_series.index).values.astype(float)
            self._raw_values[name] = raw
            # Normalize: find first non-NaN value
            first_valid = None
            for v in raw:
            ...
            if first_valid is None:
                continue
            normalized = (raw / first_valid) * 100.0
            self._series_values[name] = normalized
            color = SERIES_COLORS.get(name, "#888888")
            line = self.plot_item.plot(x, normalized, pen=pg.mkPen(color=color, width=2), name=name)
    ...
    def _render_raw(self, metals_df, settings):
        import pandas as pd
        ...
        for name in SERIES_ORDER:
            if name not in visible or name not in metals_df.columns:
                continue
            vals = metals_df[name].reindex(ref_series.index).values.astype(float)
            self._series_values[name] = vals
            self._raw_values[name] = vals
            color = SERIES_COLORS.get(name, "#888888")
    ...
    def _update_tooltip(self, idx: int, mouse_pos):
        ...
        for name in SERIES_ORDER:
            ...
            if self._view_mode == "Normalized":
                # Show indexed value + raw price
                raw_v = raw[idx] if raw is not None and idx < len(raw) else None
                price_str = f" (${raw_v:,.2f})" if raw_v is not None and not np.isnan(raw_v) else ""
                lines.append(
                    f'<span style="color:{color_str};">\u25a0</span> {name}: {v:.1f}{price_str}'
                )
            else:
                lines.append(
