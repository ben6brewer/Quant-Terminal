"""Unemployment by Age Chart — multi-line NSA rates by age group."""
...
class UnemploymentAgeChart(BaseChart):
    """Multi-line unemployment-by-age chart with recession shading."""
    ...
    def __init__(self, parent=None):
        ...
        self._visible_series: List[str] = AGE_LABELS
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading unemployment by age data...", self)
    ...
    def update_data(
        ...
        if rates_df is None or rates_df.empty:
            self.show_placeholder("No unemployment by age data available.")
            return

        available = [c for c in AGE_LABELS if c in rates_df.columns]
        if not available:
            self.show_placeholder("No age group series available.")
            return
        ...
        self._visible_series = settings.get("age_series", AGE_LABELS)

        subset = rates_df[available].dropna(how="all")
        if subset.empty:
            self.show_placeholder("Insufficient unemployment by age data.")
            return
        ...
        if self._show_recession_shading and usrec_df is not None and not usrec_df.empty:
            usrec_series = usrec_df["USREC"].reindex(dt_index, method="ffill").fillna(0)
            self._recession_bands = add_recession_bands(
                self.plot_item, usrec_series, dt_index
            )
        ...
        for name in available:
            ...
            color = AGE_COLORS.get(name, "#888888")
