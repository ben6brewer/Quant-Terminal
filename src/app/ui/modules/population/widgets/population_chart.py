"""Population Chart — US population and working-age demographics."""
...
_SERIES_COLORS = {
    "Total Population": "#4FC3F7",
    "Working Age (15-64)": "#66BB6A",
    "Civilian Population": "#FFA726",
}

_SERIES_SETTINGS = {
    "Total Population": "show_total_pop",
    "Working Age (15-64)": "show_working_age",
    "Civilian Population": "show_civilian",
}


class PopulationChart(BaseChart):
    """Population multi-line chart — Raw (millions) or YoY %."""
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading population data...", self)
    ...
    def update_data(self, pop_df: "Optional[pd.DataFrame]",
        ...
        if self._view_mode == "YoY %" and pop_df is not None and not pop_df.empty:
            growth = pop_df.pct_change(periods=12) * 100
            growth = growth.dropna(how="all")
            self._render_lines(growth, usrec_df, settings, suffix=" YoY")
        else:
            self._render_lines(pop_df, usrec_df, settings)

    def _render_lines(self, df, usrec_df, settings, suffix=""):
        import pandas as pd

        if df is None or df.empty:
            self.show_placeholder("No population data available.")
            return
