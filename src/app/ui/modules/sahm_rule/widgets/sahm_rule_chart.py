"""Sahm Rule Chart — Single-line SAHMCURRENT with 0.50 threshold."""
...
class SahmRuleChart(BaseChart):
    """Single-line Sahm Rule chart with 0.50 threshold."""
    ...
    def __init__(self, parent=None):
        ...
        self._sahm_values: Optional[np.ndarray] = None
        self._date_labels: list = []
        self._sahm_line = None
    ...
    def _setup_placeholder(self):
        self._placeholder = QLabel("Loading Sahm Rule data...", self)
    ...
    def update_data(self, sahm_df: "Optional[pd.DataFrame]",
        ...
        if sahm_df is None or sahm_df.empty:
            self.show_placeholder("No Sahm Rule data available.")
            return

        show_recession = settings.get("show_recession_bands", True)

        col = "Sahm Rule" if "Sahm Rule" in sahm_df.columns else sahm_df.columns[0]
        series = sahm_df[col].dropna()
        if series.empty:
            self.show_placeholder("No Sahm Rule data available.")
            return
        ...
        self._sahm_values = series.values.astype(float)
        ...
        show_threshold = settings.get("show_threshold", True)
        threshold_value = settings.get("threshold_value", 0.50)

        if show_threshold:
            sahm_pen = pg.mkPen(color="#FF0000", width=1, style=Qt.DashLine)
            self._threshold_line = pg.InfiniteLine(
                pos=threshold_value, angle=0, pen=sahm_pen,
                label=f"Sahm {threshold_value:.2f}",
                labelOpts={"color": "#FF0000", "position": 0.95}
            )
            self.plot_item.addItem(self._threshold_line)

        self._sahm_line = self.plot_item.plot(
            x, self._sahm_values,
            pen=pg.mkPen(color=accent, width=2.5)
        )
        self._sahm_line.setClipToView(True)

        valid = self._sahm_values[~np.isnan(self._sahm_values)]
        if len(valid) > 0:
            y_min = float(np.nanmin(valid))
            y_max = float(np.nanmax(valid))
            if show_threshold:
                y_min = min(y_min, threshold_value)
                y_max = max(y_max, threshold_value)
    ...
    def _update_tooltip(self, idx: int, mouse_pos):
        ...
        if self._sahm_values is not None and idx < len(self._sahm_values):
            val = self._sahm_values[idx]
            if not np.isnan(val):
                lines.append(
    ...
    def set_theme(self, theme: str):
        ...
        self._apply_tooltip_style()
        if self._sahm_line is not None:
            accent = self._get_theme_accent_color()
            self._sahm_line.setPen(pg.mkPen(color=accent, width=2.5))
        self._placeholder.setStyleSheet("font-size: 16px; color: #888888; background: transparent;")
