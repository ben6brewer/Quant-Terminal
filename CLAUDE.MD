...
### 6. Lazy Loading for Fast Startup

**Critical**: Heavy libraries (numpy, pandas, scipy, yfinance) must be imported inside functions, not at module level. This keeps startup under 0.5 seconds.

**Pattern 1 - Lazy Module Imports in main.py**:
```python
# In main.py - module imports inside factory functions
...
**Pattern 2 - Function-Level Heavy Imports**:
```python
# In services - import heavy libraries inside methods
...
**Pattern 3 - Lazy `__init__.py` with `__getattr__`**:
```python
...
- `main.py` - All module factory functions
- `services/__init__.py` - PEP 562 `__getattr__`
- `services/market_data.py`, `services/returns_data_service.py`
- Module services: `indicator_service.py`, `monte_carlo_service.py`, `portfolio_service.py`
- Widgets: `distribution_chart.py` (scipy)
...
│   ├── portfolio_data_service.py   # Read-only portfolio access for analysis
│   ├── returns_data_service.py     # Cached daily returns (parquet)
│   ├── favorites_service.py        # Favorites persistence
│   ├── theme_stylesheet_service.py # Centralized theme colors/styles
│   └── base_settings_manager.py    # Abstract settings persistence
│
├── ui/
...
│   │   └── common/            # ThemedDialog, DateInputWidget, EditableTableBase, etc.
│   │
│   └── modules/
...
# Use factory function (lambda) - modules are destroyed on navigate and recreated on open
hub.add_module("portfolio", lambda: PortfolioModule(theme_manager))
```
...
- `portfolios/`: Portfolio JSON files (managed by PortfolioPersistence)
- `cache/returns/`: Cached daily returns parquet files (managed by ReturnsDataService)
...
def main():
    ...
    # All modules use factory functions for lazy loading and memory efficiency
...
2. Call `hub.add_module(id, lambda: YourModule(theme_manager))` in `main.py`
3. Screenshot auto-generated on first load

### Module Lifecycle

**Important**: Modules are destroyed when navigating away and recreated when opened again.

- **Factory functions required**: Always use `lambda: Module()` not `Module()` when registering
- **No persistent state**: Module instance state is lost on navigate (use services for persistence)
- **Memory efficient**: Only one module in memory at a time (plus home screen)
- **Theme handling**: Modules apply current theme on creation, no hidden widget updates needed

```python
# Correct - factory function allows recreation
hub.add_module("charts", lambda: ChartModule(theme_manager))

# Wrong - direct instance can't be recreated after destruction
hub.add_module("charts", ChartModule(theme_manager))  # DON'T DO THIS
```

### Common Gotchas

- **Use factory functions** for `add_module()` (lambda required for module recreation)
