...
- **Module Tiles** (453×285px) display screenshot previews with emoji placeholders and star buttons
...
│   ├── config.py           # Centralized app settings, MODULE_SECTIONS
│   └── theme_manager.py    # Theme switching (dark, light, bloomberg)
...
├── ui/             # User interface
│   ├── hub_window.py       # Main application window with custom title bar
│   ├── widgets/            # Reusable UI components
│   │   ├── home_screen.py          # Central navigation hub
│   │   ├── module_tile.py          # Individual module tiles
│   │   ├── module_tile_grid.py     # 3-column tile layout
│   │   ├── section_tab_bar.py      # Section tabs with Home
│   │   └── price_chart.py          # PyQtGraph charting
│   └── modules/            # Pluggable module sections
└── utils/          # Helper functions
    └── screenshot_manager.py       # Module preview image generation
```
...
- **Fixed Size**: 453×285px (16:9 screenshot 453×255px + 30px label)
...
- **16:9 Aspect Ratio**: 453×255px images
...
### Custom Title Bar Pattern
**All new windows and dialogs should use frameless design with custom title bars:**
- Set `self.setWindowFlags(Qt.FramelessWindowHint)` in window constructor
- For dialogs, use `self.setWindowFlags(Qt.Dialog | Qt.FramelessWindowHint)`
- Create 32px title bar with app title (left) and window controls (right)
- Add minimize, maximize/restore, close buttons with theme-aware styling (main windows only)
- For dialogs, only include a close button (✕) on the right
...
**Dialog Box Implementation Pattern:**
```python
...
**Dialog Title Bar Stylesheet Pattern:**
Include these styles in your dialog's theme-specific stylesheets:
```css
#titleBar {
...
#titleBarCloseButton:hover {
    ...
    color: #ffffff;  /* White text on light theme */
}
```
...
- **Critical**: Use `setMask(QRegion)` to make overlay only "exist" where UI elements are located
- This allows widgets below to receive events directly from Qt in unmasked areas
- Update mask dynamically when overlay or its children resize/move using event filters
- **Why masking works**: Qt's `:hover` pseudo-state detection is based on physical z-order hierarchy - even if you forward events to a button, Qt won't activate `:hover` if another widget is physically on top
- **Pattern**:
  ```python
  class TransparentOverlay(QWidget):
      ...
      def eventFilter(self, obj, event):
          ...
          if obj == self._home_button and event.type() in (13, 14):  # Move, Resize
              self._update_mask()
...
**Implementation**:
- All `QPushButton` widgets automatically receive this styling via ThemeManager
- No need to set custom stylesheets on individual buttons
- Theme colors automatically adjust: Dark (cyan #00d4ff), Light (blue #0066cc), Bloomberg (orange #FF8000)

**Toggle Buttons**:
- Use `setCheckable(True)` for toggle behavior
- Checked state shows solid background to indicate active state
- Examples: Indicators panel, Depth panel, chart settings

**No Emojis**:
- Button text should be clean, no emojis (e.g., "Settings" not "⚙️ Settings")
- Rely on text labels and icon fonts if icons are needed

### Tile-Based Navigation
**Module tiles follow consistent sizing and behavior:**
- **Tile Size**: 453×285px (453×255 screenshot + 30px label)
...
- **Star Button**: Top-right corner, 32×32px, transparent background
...
### Overlay Widgets and Widget Masking
**Critical for home button overlays on modules:**
- Use widget masking (`setMask(QRegion)`) instead of event forwarding for overlays
- Masking makes the overlay physically transparent in unmasked areas, allowing widgets below to receive events directly from Qt
- This preserves Qt's built-in hover detection, which is based on physical z-order hierarchy
- **Why event forwarding doesn't work for hover states**: Qt determines `:hover` pseudo-state based on which widget is physically on top. Even if you manually forward MouseMove events to a button, Qt won't activate the button's `:hover` state if another widget (the overlay) is physically positioned above it in the z-order
- Update masks dynamically using event filters when overlay children move/resize
- See "Transparent Overlays with Widget Masking" section above for implementation pattern
...
2. **Add to CONFIG**: Add entry to `MODULE_SECTIONS` in `core/config.py`:
   ```python
   "Section Name": [
       {"id": "module_id", "label": "Module Label", "emoji": "📊"}
   ]
   ```
...
- **Tile Layout**: 3 columns, 453×285px tiles, 20px spacing
...
- Widget masking with `setMask(QRegion)` preserves Qt's hover detection for overlays - event forwarding doesn't work for `:hover` pseudo-states
